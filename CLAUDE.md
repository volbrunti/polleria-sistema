...
1. ✅ **Backend completo del módulo 1** con tests — TERMINADO (74/74 tests, ver README.md y HANDOFF-AUDITORIA.md).
...
- **Precio**: producto, monto, fechaDesde, usuario. Historial: nunca se pisa, cambio = registro nuevo. Funciona igual para un `COMBO` que para cualquier otro producto — el precio del combo es este mismo mecanismo, no una entidad aparte.
...
- **Carta real del cliente — carga pendiente**: el cliente pasó 5 fotos del menú actual (POLLO C/FRITAS con combos y promos, MILAS, LOMOS, BURGERS, papas/ensaladas/escabeches/empanadas) pero **todavía faltan 2 imágenes**. Se decidió esperar el menú completo antes de cargar los productos/precios individuales al catálogo — el modelo de `COMBO` ya está listo (ver §9) para cuando llegue el resto.
...
6. Todos los tests pasan (74/74), incluidos los de no-filtración de campos ciegos, RBAC, aislamiento de sucursal y combos.