...
- ✅ **Endpoint de trazabilidad por pedido (2026-07-26)**: `GET /api/reportes/trazabilidad/pedido/:id` (ADMIN+SOCIO), `reportesService.trazabilidadPedido()`. Reconstruye, para cada ítem del pedido (o cada componente si es COMBO): la transferencia que trajo el producto a la sucursal, el lote de producción de origen, sus insumos usados y el `LineaIngreso`/proveedor de cada uno. **Limitación de diseño conocida y documentada en el código**: el schema no tiene un FK directo entre `LoteDeProduccion` y `Transferencia` (los envíos no reservan unidades de un lote específico), así que el lote de origen se infiere por proximidad temporal (el último lote CERRADO de ese producto antes del envío) — es una aproximación FIFO razonable, no una relación exacta. La respuesta marca `lote.aproximado: true` para dejarlo explícito. Si no hay transferencia registrada (ej. "Pollo a la leña — MARCADO", que se genera localmente por `EventoMarcadoPollo` y nunca se transfiere) la respuesta no se rompe: vuelve `transferencia: null` con una nota explicativa. Frontend: pestaña "Trazabilidad" en `Reportes.tsx` con buscador por número de pedido. Sin tests de integración todavía — bloqueado por falta de acceso a la DB de test desde este entorno (mismo bloqueo que 2.1 del checklist pre-go-live).
