...
- ⬜ Fases 5–9: pendientes. Pendientes puntuales: (i) sala de Socket.io por sucursal para que `turno:desbloqueado` llegue al POS del cajero (Fase 6 — hoy solo sala de admins); (ii) **el retorno a producción no genera `LineaIngreso`**: producción ve el stock retornado pero no puede consumirlo por partida en un lote (el módulo 1 exige partida de origen) — definir con el cliente si el retorno debe crear una partida propia ("retorno del local X del día D") cuando se pruebe el flujo real.
