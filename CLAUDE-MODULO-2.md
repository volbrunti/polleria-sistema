...
- ⬜ Fase 9 (pulido final): (i) **el retorno a producción no genera `LineaIngreso`**: producción ve el stock retornado pero no puede consumirlo por partida en un lote (el módulo 1 exige partida de origen) — **PREGUNTA BLOQUEANTE PARA EL MÓDULO 3** (sus reportes de trazabilidad dependen de LineaIngreso): ¿el retorno crea una partida propia con proveedor especial "Retorno interno", o se acepta trazabilidad parcial?; (ii) **modificar pedido confirmado (PATCH /pedidos/:id) no tiene UI todavía** — el backend está listo y testeado; en el POS hoy se resuelve anulando y recreando (repone y descuenta stock correctamente, pero genera ticket de anulación + ticket nuevo en cocina — hacer la pantalla antes del go-live); (iii) los motivos de atención (`MOTIVOS_ATENCION` en `frontend/src/api/caja.ts`) son una lista nuestra no validada con el cliente; (iv) timer de `pedido:listo_no_retirado` (§9) sin implementar — default sugerido 30 min, confirmar con cliente.
