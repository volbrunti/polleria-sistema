...
- ⬜ Fase 9 (pulido final): pendientes puntuales: (i) sala de Socket.io por sucursal para que `turno:desbloqueado`, `alerta:stock_minimo` y los tickets lleguen al POS/cocina del local en vivo (hoy: sala de admins + polling en el POS); (ii) **el retorno a producción no genera `LineaIngreso`**: producción ve el stock retornado pero no puede consumirlo por partida en un lote (el módulo 1 exige partida de origen) — definir con el cliente si el retorno debe crear una partida propia ("retorno del local X del día D") cuando se pruebe el flujo real; (iii) **modificar pedido confirmado (PATCH /pedidos/:id) no tiene UI todavía** — el backend está listo y testeado; en el POS hoy se resuelve anulando y recreando (repone y descuenta stock correctamente); (iv) los motivos de atención (`MOTIVOS_ATENCION` en `frontend/src/api/caja.ts`) son una lista nuestra no validada con el cliente.
