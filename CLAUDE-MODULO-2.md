...
- 🔶 Fase 9 (pulido final) — parcialmente resuelta el 2026-07-24 (informe de auditoría de negocio, contrastado contra el código real antes de aplicar nada):
  ...
  - ⬜ **Sigue pendiente — bloqueante para el módulo 3**: el retorno a producción no genera `LineaIngreso`. Producción ve el stock retornado pero no puede consumirlo por partida en un lote (el módulo 1 exige partida de origen). Pregunta ya redactada en `preguntas-pablo-whatsapp.txt`, sin enviar todavía: ¿el retorno crea una partida propia con proveedor especial "Retorno interno", o se acepta trazabilidad parcial?
...
- Retorno a producción sin `LineaIngreso` (ver §0 Fase 9): pregunta bloqueante para el módulo 3, sin responder.
