...
const crearSchema = z.object({
  ...
  tipo: z.enum(['EMPLEADO', 'ENCARGADO']),
...
export async function descuentosRoutes(app: FastifyInstance) {
  ...
  // dato ciego — el descuento se lo aplican al cliente y va en el ticket.
  app.get(
    '/',
  ...
  app.post('/', { preHandler: [...soloAdmin] }, async (req, reply) => {
    ...
    const descuento = await descuentosService.crear(datos, req.usuario.id);
  ...
  app.patch('/:id', { preHandler: [...soloAdmin] }, async (req) => {
    ...
    return descuentosService.actualizar(id, datos, req.usuario.id);
  });
}
